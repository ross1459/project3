...
create view users_view as select users.name,
...
create or replace function get_survey(_name integer)
returns integer as
$func$
declare
rec record;
begin
select into rec * from surveys where s_id = _name;
if not found then
return -1;
elseif (_name = rec.id) then
return rec.id;
else
return -1;
end if;
